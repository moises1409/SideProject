...
export class VideoGenerationComponent {
  ...
  private generateVideo() {
    ...
    const apiUrl = this.getApiUrl('/auto_editor');
  ...
  private pollForVideoAvailability(taskId: string): void {
    const apiUrl = this.getApiUrl(`/task_status/${taskId}`);
  ...
  private generateStory(): Observable<StoryResponse | undefined> {
    ...
    const apiUrl = this.getApiUrl('/get_story');
  ...
  generateImage(parameter: string): Observable<string | undefined> {
    const apiUrl = this.getApiUrl('/get_image');
  ...
  generateAudio(parameter: string): Observable<string | undefined> {
    const apiUrl = this.getApiUrl('/get_audio');
    const params = new HttpParams()
      .set('text', parameter)
      .set('language', this.selectedLanguage);
  ...
  private deleteAudioFiles(audioUrls: string[]): void {
    const apiUrl = this.getApiUrl('/delete_audio_files'); // Ensure this matches your backend URL
